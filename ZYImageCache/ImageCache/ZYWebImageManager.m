...
#import "ZYWebImageDownloadOperation.h"
...
+ (instancetype)sharedManager{
    static ZYWebImageManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[ZYWebImageManager alloc] init];
    });
    return manager;
}
...
- (void)downloadImageWithUrlString:(NSString *)urlString completeHandle:(KCCompleteHandleBlock)completeHandle title:(NSString *)title{
    ...
    // 下面就是创建操作 下载 --- 自定义
    ZYWebImageDownloadOperation *downOp = [[ZYWebImageDownloadOperation alloc] initWithDownloadImageUrl:urlString completeHandle:^(NSData *imageData,NSString *kc_urlString) {
...
- (void)cancelDownloadImageWithUrlString:(NSString *)urlString{
    ...
    ZYWebImageDownloadOperation *op = self.operationDict[urlString];
    [op cancel];
